// The full attention pipeline with all four weight matrices random, so scores
// are non-uniform and softmax actually has to discriminate.
//
...
//     always addr=0
//   * output rows that DIFFER from each other -- every earlier test produced
//     16 identical rows, which would hide any per-row indexing bug
//
...
class tc_attention_full extends tpu_base_test;
    `uvm_component_utils(tc_attention_full)
    ...
    const int W_Q_BASE   = 32'h0000;
    const int W_K_BASE   = 32'h1000;
    ...
    function new(string name = "tc_attention_full", uvm_component parent = null);
